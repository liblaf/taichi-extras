...
E, nu = 1e4, 0.0
GRAVITY: ti.Vector = ti.Vector([0.0, -9.8, 0.0])
...
def init(mesh_filepath: Path) -> tuple[ti.MeshInstance, ti.ScalarField]:
    ...
    fixed.from_numpy(
        np.loadtxt(fname=mesh_filepath.with_suffix(".fixed.txt"), dtype=np.int8)
    )
...
def main(
    ...
    with taichi_extras.patches.ui.window.Window(
        ...
        while w.next_frame(max_frames=max_frames):
            ...
            scene.mesh(
                vertices=mesh.verts.get_member_field(key="position"), indices=indices
            )
